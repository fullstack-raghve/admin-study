...
import { MatSnackBar, MatTabGroup, MatDialog, MatDialogConfig } from '@angular/material';
...
import { AddFlowNameComponent } from '../add-flow-name/add-flow-name.component';
...
export class CreateFlowComponent implements OnInit {
  ...
  constructor(
    ...
    private dataService: SendJsonDatatoParentService,
  ...
  getSentimentNPSValues() {
    let GET_EMOJI_NPS =
      environment.APIEndpoint + "api/rpa/feedback/nps/sentiment/v1/view";
    this.http.getJson(GET_EMOJI_NPS).subscribe(response => {
      ...
      //           this.emojiValues[j].label = this.sentimentIcons[
      //             k
      //           ].beforeClickSentimentImage;
      //           this.emojiValues[
      //             j
      //           ].afterClickSentimentImage = this.sentimentIcons[
      //             k
      //           ].afterClickSentimentImage;
      //         }
      //       }
      //     }
      //     console.log(this.emojiValues);
    ...
    if (this.data) {
      ...
      if (this.data.attributes.length > 0) {
        for (var i = 0; i > this.data.attributes.length; i++) {
          console.log(
            "customerdata",
            this.data.attributes["customerData"].type
          );
        }
      }
      const found = this.pages.some(el => el.pageNo === this.data.pageNo);
      console.log("index " + found);
      if (!found) {
        this.pages.push(this.getJsonDataCreateFlow);
      } else {
        this.pages[this.data.pageNo - 1] = this.getJsonDataCreateFlow;
      }
  ...
  public buildCreateAboutUsForm() {
    ...
    this.createFlowFormGroup = this.fb.group(form);
  }
  ...
  removeTab(index: number) {
    ...
    let indesx = this.pages.findIndex(record => record.pageNo === index + 1);
    let indexNew = this.tabs1.findIndex(record => record.pageNo === index);
    console.log(index);
    this.pages.splice(indesx, 1);
    console.log("pages", this.pages);
    this.tabs1.splice(indexNew, 1);
    console.log("pages", this.tabs1);
  }
  //tabs--
  ...
  onDrop(i, event: DndDropEvent, list?: any[], currentIndex?: any, currentPageinfo?: any) {
    ...
    if(this.createFlowFormGroup.controls['languageCode'].value == 'AR'){
  ...
  createFlow(formdata) {
    ...
    if (this.createFlowFormGroup.invalid == true) {
      this.showError = true;
      this.loading = false;
    } else {
      ...
      if (this.tabs1[this.mainIndex]['attributes'].length === 0) {
        ...
        const req = {
          ...
          kioskType: formdata.kiosk_Type === true ? "YES" : "NO",
          smsType: formdata.sms_Type === true ? "YES" : "NO",
          emailType: formdata.email_Type === true ? "YES" : "NO",
          onlineType: formdata.online_type === true ? "YES" : "NO",
          ...
          isQrCodeEnquiry:formdata.feedback_flow_for === 'QR_CODE' ? 1 : 0,
          isFbTransactionPickup:formdata.feedback_flow_for === 'FNB_TRANSACTION_PICK_UP' ? 1 : 0,
          isFbTransactionDelivery:formdata.feedback_flow_for === 'FNB_TRANSACTION_DELIVERY' ? 1 : 0,
          isQrTransactionPickup:formdata.feedback_flow_for === 'QR_CODE_PICK_UP' ? 1 : 0,
          isQrTransactionDelivery:formdata.feedback_flow_for === 'QR_CODE_DELIVERY' ? 1 : 0,
        ...
        this.http
          .postJson(
            environment.APIEndpoint + "api/rpa/feedback/flow/v1/createFlow",
            req
          )
          .subscribe(
            res => {
            ...
            err => {
              ...
              this.disabled = false;
              console.log(
                "error Status = " + err.error.errorDetails[0].description
              );
  ...
  getBrand() {
    ...
    this.http.getJson(environment.APIEndpoint + SEARCH_BRANDS).subscribe(
      res => {
        // this.brandList = res;
  ...
  getCountryByBrandId(BrandId) {
    const COUNTRY_BY_BRAND =
      ...
      BrandId;
    this.http.getJson(COUNTRY_BY_BRAND).subscribe(response => {
      console.log(response);
      this.countries = response;
    });
  }
  ...
  uploadSkuFile(event: FileList) {
    if (
      event[0].type == "application/vnd.ms-excel" ||
      event[0].type ==
      "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" ||
      event[0].type == "application/json"
    ) {
      this.uploadFile.upload(event.item(0), this.moduleName, "files").subscribe(
        response => {
  ...
  setAllIndexToPageNumber() {
    if (this.tabs1.length > 1) {
      this.getAllIndexNumber.forEach((element, i) => {
  ...
  editPageName(i) {
    ...
    let dialogRef = this.dialog.open(AddFlowNameComponent, dialogConfig);

    dialogRef.afterClosed().subscribe(result => {
      if (result != undefined) {
        console.log(result);
        this.tabs1[result.index]['pageName'] = result.value;
      }
    });
  }
  ...
  all = [
    { "pageNo": 1, "attributes": [], "pageName": "page 1", "showTab": false, "buttons": [], "value": [{ "pageNumber": 1, "edit": false }], "hover": false },
     { "pageNo": 2, "pageName": "page 2", "showTab": false, "attributes": [], "buttons": [], "value": [{ "pageNumber": 2, "edit": false }], "hover": false }, 
     { "pageNo": 3, "pageName": "page 3", "showTab": false, "attributes": [{ "type": "textarea", "icon": "assets/images/flow_icons/large_text.png", "question": "Large Text", "className": "form-control", "subType": "textarea", "linkTo": "", "tempVariable": "", "handle": true, "name": "textarea-1584980351463" },
      { "type": "popup", "icon": "assets/images/flow_icons/popup.png", "question": "Popup", "subType": "popup", "linkTo": "", "className": "form-control", "popupDescription": "", "tempVariable": "", "name": "popup-1584980409477" }], "buttons": [], "value": [{ "pageNumber": 3, "edit": false }], "hover": false },
       { "pageNo": 4, "pageName": "page 4", "showTab": false, "attributes": [], "buttons": [], "value": [{ "pageNumber": 4, "edit": false }], "hover": false }, { "pageNo": 5, "pageName": "page 5", "showTab": false, "attributes": [], "buttons": [], "value": [{ "pageNumber": 5, "edit": false }], "hover": false }, { "pageNo": 6, "pageName": "page 6", "showTab": true, "attributes": [{ "type": "textarea", "icon": "assets/images/flow_icons/large_text.png", "question": "Large Text", "className": "form-control", "subType": "textarea", "linkTo": "", "tempVariable": "", "handle": true, "name": "textarea-1584980402672" }], "buttons": [], "value": [{ "pageNumber": 6, "edit": false }], "hover": false }]
}
