...
export class AddBrandComponent implements OnInit {
  ...
  checked = true;
  disabled = false;
  @ViewChild("createBrandForm") createBrandForm;
  ...
  public buildCreateCityForm() {
    let form = {
      brandLocale: this.fb.array([]),
      brandCode: ["", Validators.compose([Validators.required, Validators.pattern("^[A-Za-z0-9]{1,20}$")])],
  ...
  public brandLocale() {
    ...
    const control = <FormArray>this.brandFormGroup.controls['brandLocale'];
    for (let ln of this.languageList) {
      let arr = this.fb.group({
        brandName: ['', Validators.compose([Validators.required, Validators.minLength(2), Validators.pattern('[a-zA-Z\u0600-\u06FF \"\'&(),.-:?_]*')])]
      });
      control.push(arr);
  ...
  createBrand(formData) {
    ...
    if (this.brandFormGroup.invalid == true) {
      ...
      formData.brandLocale.forEach((city, index) => {
        this.brandLocales.push({
      ...
      let createCityReq = {
        brandLocales: this.brandLocales,
