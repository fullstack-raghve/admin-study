...
@Component({
  selector: 'varient-dialog',
  templateUrl: './varient-dialog.component.html',
  styleUrls: ['./varient-dialog.component.scss']
})
    ...
    public productVariantsForm() {
      ...
      for (let p = 0; p < this.popUPData.length; p++) {
        const newForm = this.fb.group({
          isVariantTypeSelected: [this.popUPData[p].isVariantTypeSelected],
          variantTypeName: [this.popUPData[p].variantTypeName],
          variantTypeOid: [this.popUPData[p].variantTypeOid],
          maxSelectionCustAddons:[this.popUPData[p].maxSelectionCustAddons || 0],
          maxQuantityPerAddons:[this.popUPData[p].maxQuantityPerAddons || 0],
        });
