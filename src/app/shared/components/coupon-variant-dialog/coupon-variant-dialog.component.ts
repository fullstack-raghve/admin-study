import { OnInit, Input, Component, ViewChild, ElementRef, EventEmitter, Output } from "@angular/core";
...
@Component({
  selector: 'app-coupon-variant-dialog',
  templateUrl: './coupon-variant-dialog.component.html',
  styleUrls: ['./coupon-variant-dialog.component.scss']
})
export class CouponVariantDialogComponent implements OnInit {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<CouponVariantDialogComponent>, private http: HttpService,
  ) {
    ...
    for (let j of data.groupItem) {
      ...
      if (allSelected.length != 0) {
        for (let k = 0; k < allSelected.length; k++) {
          if (j.skuCode == allSelected[k]['skuCode']) {
            value = true;
            break;
          }
        }
    ...
    for (let j of data.groupItem) {
      ...
      if (this.selectedSkuProduct.length != 0) {
        for (let k = 0; k < this.selectedSkuProduct.length; k++) {
          if (j.skuCode == this.selectedSkuProduct[k]['skuCode']) {
            value = true;
            break;
          }
        }
  ...
  getSkuProducts() {
    ...
    const GET_COUPON_PRODUCTS = environment.APIEndpoint + "api/rpa/product/v2/searchProduct";
    this.http.postJson(GET_COUPON_PRODUCTS, request)
      .subscribe((response) => {
        ...
        if (response['items'].length > 0) {
          ...
          this.prodGroupArr = this.productList.reduce((r, { productName }) => {
            if (!r.some(o => o.productName == productName)) {
              r.push({ productName, groupItem: this.productList.filter(v => v.productName == productName) });
            }
            return r;
          }, []);
  ...
  addProduct(value) {
    ...
    let filterdProd = area.reduce((unique, o) => {
      if (!unique.some(obj => obj.skuCode === o.skuCode)) { 
        unique.push(o);
      }
      return unique;
    }, []);
    ...
    if (filterdProd.length > 0) {
      ...
      this.dialogRef.close(filterdProd);
    } else {
  ...
  onCloseClick() {
    this.dialogRef.close(this.selectedSkuProduct);
  }
